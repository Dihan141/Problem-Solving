    ...
    for (i = 0; i <= fdcount; i++)  // Checking each functional dependancy
    {
      ...
      if (issubset == 1) {
...

        {
          ...
          {
            if (closure[j] == f[i].right[k]) found = 1;
          }
          if (found == 0)
...
void get_ronly() {
  ...
  for (i = 0; i < attrcount; i++)  // take an atrribute
  {
    ...
    if (lpresent == 0 && rpresent == 1) ronly[l++] = i;
  }
  ronly[l] = 222;
}
...
void get_lonly() {
  ...
  for (i = 0; i < attrcount; i++)  // take an atrribute
  {
    ...
    if (lpresent == 1 && rpresent == 0) lonly[l++] = i;
  }
  lonly[l] = 222;
}
...
void get_merg1n3() {
  ...
  int i, j;
  for (i = 0, j = 0; lonly[j] != 222; i++, j++) merg1n3[i] = lonly[j];
  for (j = 0; nolnor[j] != 222; i++, j++) merg1n3[i] = nolnor[j];
...
int compare(char temp[25]) {
  ...
  for (i = 0; i < attrcount; i++) {
    if (strcmp(temp, attr[i]) == 0) return i;
  }
  return 0;
}
...
int main() {
  ...
  for (i = 0; i < 10; i++) {
    scanf("%s", temp1);
    if (strcmp(temp1, "0") == 0) break;
    fdcount++;
    j = 0;
    if (temp1[0] == '{' || temp1[0] == '(') j++;
    do {
      if (temp1[j] == ',') j++;
      k = 0;
  ...
  i = iscomplete();  // check whether
   ...
   if (i == 1) {
    ...
    /************** Step 6 **************
    6. Find the relation exteriors, that is the attrbutes included not in 4 and
    not in 2 included not in (ronly & merg1n3)
         */
    k = 0;
    for (i = 0; i < attrcount; i++) {
      found = 0;
    ...
    /************** Step 6 **************
    7. Now test the closures of attributes on step 4 + one attribute in step 6
    one at a time.
    */
